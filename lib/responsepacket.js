  ...
  if (res.type === consts.CHALLENGE_TYPE) {
    ...
    if (r !== 'splitnum') {
      ...
      while (bc.buffer.readUInt16LE(bc.tell()) !== KEYVAL_END) {
        key = readString(bc)
        value = readString(bc, key === 'hostname' ? validKeys : null)
...
ResponsePacket.write = function (packet) {
  ...
  var bc = new BufferCursor(Buffer.alloc(MAX_PACKET_SIZE))
...
function readString (bc, followedBy = null) {
  var start = bc.tell()
  var b = bc.readUInt8()
  while (bc.tell() < bc.length && (b !== 0x0 || !stringFollowedBy(bc.buffer, bc.tell(), followedBy))) {
    b = bc.readUInt8()
  }
...
function stringFollowedBy (buffer, offset, options) {
  if (!options || !options.length) {
    return true
  }
  for (let option of options) {
    if (buffer.byteLength - offset < option.byteLength) {
      continue
    }
    let data = Buffer.from(buffer.buffer, buffer.byteOffset + offset, option.byteLength)
    if (Buffer.compare(data, option) === 0) {
      return true
    }
  }
  return false
